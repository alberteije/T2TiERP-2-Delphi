...
uses
  ...
  uniImageList, uniLabel, Menus, uniMainMenu, pngimage, uniImage, Data.DBXMySQL,
  Data.DB, Data.SqlExpr, Datasnap.DBClient, uniBasicGrid, uniDBGrid;
...
type
  TMainForm = class(TUniForm)
    ...
    CDSCarrinhoNOME: TStringField;
    CDSCarrinhoQUANTIDADE: TFloatField;
    CDSCarrinhoVALOR: TFloatField;
    CDSCarrinhoGTIN: TStringField;
    CDSCarrinhoTOTAL: TAggregateField;
...
begin
  ...
  try
    ...
    for I := 0 to C - 1 do
      Ini.WriteString('favorites', IntToStr(I), Favorites1.Items[I].Caption);
  finally
    Ini.Free;
  end;
end;
...
begin
  ...
  begin
    if Favorites1.Find(ANode.Text) =  nil then
    begin
...
var
  M : TUniMenuItem;
begin
  M := Favorites1.Find(NavTree.Selected.Text);
  if Assigned(M) then
  begin
    M.Free;
    WriteFavs;
