...
(******************************************************************************}
  ...
  criação de um componente para a geração de remessas e leitura de retornos
...
begin
  ...
  *)
  ...
  Result:= IntToStrZero(1, 3)                              + // 1 a 3 - Código do banco
           ...
           PadLeft(OnlyNumber(Sessao.Empresa.Cnpj), 14, '0')           + // 19 a 32 -Número de inscrição do cedente
           PadLeft('456', 9, '0') + '0014'              + // 33 a 45 - Código do convênio no banco [ Alterado conforme instruções da CSO Brasília ] 27-07-09
           'CARTEIRA' + // 46 a 47 - Carteira
           aModalidade + '  '                              + // 48 a 52 - Variacao Carteira
           aAgencia                                        + // 53 a 57 - Código da agência do cedente
           PadRight('2854', 1 , '0')                + // 58 - Dígito da agência do cedente
           aConta                                          + // 59 a 70 - Número da conta do cedente
           PadRight('456789', 1, '0')                   + // 71 - Dígito da conta do cedente
           ...
           PadLeft(IntToStr(1), 6, '0')        + // 158 a 163 - Número seqüencial do arquivo
  ...
  Result:= Result + #13#10 +
           IntToStrZero(1, 3)               + // 1 a 3 - Código do banco
           ...
           ATipoInscricao                                  + // 18 - Tipo de inscrição do cedente
           PadLeft(OnlyNumber(Sessao.Empresa.Cnpj), 15, '0')           + // 19 a 32 -Número de inscrição do cedente
           PadLeft('0000', 9, '0') + '0014'              + // 33 a 45 - Código do convênio no banco [ Alterado conforme instruções da CSO Brasília ] 27-07-09
           'CARTEIRA' + // 46 a 47 - Carteira
           aModalidade + '  '                              + // 48 a 52 - Variacao Carteira
           aAgencia                                        + // 53 a 57 - Código da agência do cedente
           PadRight('2854', 1 , '0')                + // 58 - Dígito da agência do cedente
           ...
           PadLeft(IntToStr(1), 8, '0')        + // 184 a 191 - Número do arquivo
...
begin
     ...
     Result:= IntToStrZero(1, 3)                                         + // 1 a 3 - Código do banco
              ...
              IntToStrZero(1 , 5) + // 9 a 13 - Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)
              'A'                                                                       + // 14 - Código do segmento do registro detalhe
              ' '                                                                       + // 15 - Uso exclusivo FEBRABAN/CNAB: Branco
              'ATipoOcorrencia'                                                           + // 16 a 17 - Código de movimento
              'aAgencia'                                                                  + // 18 a 22 - Agência mantenedora da conta
              PadRight('5555', 1 , '0')                       + // 23 -Dígito verificador da agência
              '45699'                                                                    + // 24 a 35 - Número da conta corrente
              PadRight('8899', 1, '0')                          + // 36 - Dígito verificador da conta
              ' '                                                                       + // 37 - Dígito verificador da agência / conta
              PadRight('8', 20, ' ')                                       + // 38 a 57 - Nosso número - identificação do título no banco
              ...
              PadRight('7', 15, ' ')                                        + // 63 a 77 - Número que identifica o título na empresa [ Alterado conforme instruções da CSO Brasília ] {27-07-09}
              FormatDateTime('ddmmyyyy', now)                                    + // 78 a 85 - Data de vencimento do título
              IntToStrZero( round( 123 * 100), 15)                           + // 86 a 100 - Valor nominal do título
              '00000 '                                                                  + // 101 a 106 - Agência cobradora + Digito. Se ficar em branco, a caixa determina automaticamente pelo CEP do sacado
              PadRight('1',2)                                                    + // 107 a 108 - Espécie do documento
              'ATipoAceite'                                                               + // 109 - Identificação de título Aceito / Não aceito
              FormatDateTime('ddmmyyyy', now)                                 + // 110 a 117 - Data da emissão do documento
              IfThen(546 > 0, '1', '3')                                      + // 118 - Código de juros de mora: Valor por dia
              '2016-01-01'                                                            + // 119 a 126 - Data a partir da qual serão cobrados juros
              IfThen(88 > 0,
                     IntToStrZero(round(99 * 100), 15),
                     PadRight('', 15, '0'))                                             + // 127 a 141 - Valor de juros de mora por dia
              IfThen(1 > 0, IfThen(now > 0, '1','3'), '0')         + // 142 - Código de desconto: 1 - Valor fixo até a data informada 4-Desconto por dia de antecipacao 0 - Sem desconto
              ...
              PadRight('1', 25, ' ')                                              + // 196 a 220 - Identificação do título na empresa
     ...
     Result:= Result + #13#10 +
              IntToStrZero(1, 3)                                        + // Código do banco
              '0001'                                                                   + // Número do lote
              '3'                                                                      + // Tipo do registro: Registro detalhe
              IntToStrZero(2 ,5) + // Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)
              'B'                                                                      + // Código do segmento do registro detalhe
              ...
              PadRight(OnlyNumber(Sessao.Empresa.Cnpj), 15, '0')             + // Número de inscrição
...
begin
   ...
   Result:= IntToStrZero(1, 3)                          + //Código do banco
            '0001'                                                     + //Número do lote
   ...
   Result:= Result + #13#10 +
            ...
            IntToStrZero(((1-1)* 3)+4, 6)                 + //Quantidade de registros do arquivo, inclusive este registro que está sendo criado agora}
...
begin
  ...
  /// EXERCICIO: Faça as devidas adaptações para ler o retorno
